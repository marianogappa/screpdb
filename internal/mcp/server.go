...
func NewServer(storage storage.Storage) *Server {
	...
	sqlTool := mcp.NewTool("query_database",
		mcp.WithDescription("Run a read-only SQL query (SELECT/WITH/EXPLAIN/PRAGMA only) against the StarCraft: Remastered replay database and get the rows back. Tables: replays (one row per game: map, matchup, duration, engine), players (one row per player per replay: race, APM/eAPM, is_winner, start location, hotkey_stream — an encoded BLOB of hotkey usage, not meaningful in SQL), commands (the ordered action stream — builds, trains, morphs, tech, upgrades, micro), commands_low_value (high-volume noise: right-clicks, pings — usually excluded), replay_events (derived analysis: build-order openers, timing markers, and narrative game events like rushes/drops/proxies), player_fingerprint_vectors (per-player playstyle feature vectors as float64 BLOBs — metadata like feature_version/race/cmd_count is queryable, the vector blob itself is not meaningful in SQL). Call get_database_schema for exact columns and get_starcraft_knowledge for domain terms before writing non-trivial queries."),
		mcp.WithString("sql",
			mcp.Required(),
...
func (s *Server) handleGetSchema(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	- commands vs commands_low_value: high-signal actions (Build, Train, morphs, Tech, Upgrade, targeted micro) live in commands; high-volume noise (Right Click, Minimap Ping, Vision, Alliance) is split into commands_low_value so it can be excluded from analysis. Same schema in both. Right-clicks are only stored if ingestion was configured to keep them, so don't assume they exist. Hotkey commands are never stored as rows: each player's hotkey history lives in players.hotkey_stream, a delta-encoded varint BLOB (not meaningful in SQL; NULL means the player used no hotkeys).
	- replay_events is the DERIVED analysis layer (not raw stream). event_kind = 'marker' rows are one-per-(replay, player, event_type) summaries screpdb computed — build-order openers are stored as event_type feature keys prefixed 'bo_' (e.g. bo_9_pool, bo_12_hatch, bo_gate_expand, bo_t_111); opener_unresolved / *_fuzzy / bo_*_other are catch-alls. Other markers are timings/behaviours (e.g. never_used_hotkeys, viewport_multitasking, never_upgraded). event_kind = 'game_event' rows are narrative moments (rushes, drops, proxies, nydus, mind control, scout, expansion). source_player_id/target_player_id join to players.id; location_base_type ('starting'|'natural'|'expansion') and location_base_oclock give map position; payload is optional JSON. To discover the actual event_type values, use the list_event_types tool or: SELECT event_kind, event_type, COUNT(*) FROM replay_events GROUP BY 1,2 ORDER BY 3 DESC.
