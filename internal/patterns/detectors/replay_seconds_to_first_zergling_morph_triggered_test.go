...
func TestSecondsToFirstZerglingMorphTriggeredReplayDetector(t *testing.T) {
	tests := []struct {
		name           string
		...
		{
			name: "detects first zergling morph from any player at 45 seconds",
			commands: []commandSpec{
				{playerID: 1, action: models.ActionTypeUnitMorph, unit: models.GeneralUnitZergling, seconds: 45},
			},
			wantFinished:   true,
		...
		},
		{
			...
			commands: []commandSpec{
				{playerID: 2, action: models.ActionTypeUnitMorph, unit: models.GeneralUnitZergling, seconds: 40},
				{playerID: 1, action: models.ActionTypeUnitMorph, unit: models.GeneralUnitZergling, seconds: 50},
			},
			wantFinished:   true,
		...
		},
		{
			...
			commands: []commandSpec{
				{playerID: 1, action: models.ActionTypeUnitMorph, unit: "Drone", seconds: 45},
			},
			wantFinished:   false,
			wantResult:     nil,
		...
		},
		{
			...
			commands: []commandSpec{
				{playerID: 1, action: models.ActionTypeBuild, unit: models.GeneralUnitZergling, seconds: 45},
			},
			wantFinished:   false,
			wantResult:     nil,
			wantShouldSave: false,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			detector := NewSecondsToFirstZerglingMorphTriggeredReplayDetector()
