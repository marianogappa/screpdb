...
func TestSecondsToFirstGatewayBuildTriggeredReplayDetector(t *testing.T) {
	tests := []struct {
		name           string
		...
		{
			name: "detects first gateway build from any player at 60 seconds",
			commands: []commandSpec{
				{playerID: 1, action: models.ActionTypeBuild, unit: models.GeneralUnitGateway, seconds: 60},
			},
			wantFinished:   true,
		...
		},
		{
			...
			commands: []commandSpec{
				{playerID: 2, action: models.ActionTypeBuild, unit: models.GeneralUnitGateway, seconds: 50},
				{playerID: 1, action: models.ActionTypeBuild, unit: models.GeneralUnitGateway, seconds: 70},
			},
			wantFinished:   true,
		...
		},
		{
			...
			commands: []commandSpec{
				{playerID: 1, action: models.ActionTypeBuild, unit: "Nexus", seconds: 60},
			},
			wantFinished:   false,
			wantResult:     nil,
		...
		},
		{
			...
			commands: []commandSpec{
				{playerID: 1, action: models.ActionTypeTrain, unit: models.GeneralUnitGateway, seconds: 60},
			},
			wantFinished:   false,
			wantResult:     nil,
			wantShouldSave: false,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			detector := NewSecondsToFirstGatewayBuildTriggeredReplayDetector()
