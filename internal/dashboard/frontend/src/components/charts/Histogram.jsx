...
function Histogram({ data, config }) {
  ...
  useEffect(() => {
    ...
    const binsData = d3.bin().domain(d3.extent(values)).thresholds(bins)(values);
    if (binsData.length === 0) return;

    const xScale = d3.scaleLinear().domain([binsData[0].x0, binsData[binsData.length - 1].x1]).range([0, width]);
    const yScale = d3.scaleLinear().domain([0, d3.max(binsData, d => d.length)]).range([height, 0]);

    svg.append('g').attr('transform', `translate(0, ${height})`).call(d3.axisBottom(xScale).tickSizeOuter(0)).selectAll('text').attr('fill', '#fff');
    svg.append('g').call(d3.axisLeft(yScale)).selectAll('text').attr('fill', '#fff');

    svg.selectAll('.bar').data(binsData).enter().append('rect')
      .attr('class', 'bar')
    ...
    svg.append('text').attr('text-anchor', 'end').attr('x', width).attr('y', height + 35)
      .attr('fill', '#fff').attr('font-size', '12px').text(config?.histogram_value_column);
