...
function ScatterPlot({ data, config }) {
  ...
  useEffect(() => {
    ...
    const xScale = d3.scaleLinear().domain(d3.extent(data, d => Number(d[config.scatter_x_column]) || 0)).range([0, width]);
    const yScale = d3.scaleLinear().domain(d3.extent(data, d => Number(d[config.scatter_y_column]) || 0)).range([height, 0]);

    const sizeScale = config.scatter_size_column
      ? d3.scaleSqrt().domain(d3.extent(data, d => Number(d[config.scatter_size_column]) || 1)).range([3, 15])
      : () => 5;

    const colorScale = config.scatter_color_column
      ? d3.scaleOrdinal(DEFAULT_COLORS).domain([...new Set(data.map(d => String(d[config.scatter_color_column])))])
      : () => DEFAULT_COLORS[0];
    ...
    svg.selectAll('.dot').data(data).enter().append('circle')
      ...
      .attr('opacity', 0.6)
      .on('mouseover', function () { d3.select(this).attr('opacity', 1).attr('r', d => sizeScale(config.scatter_size_column ? d[config.scatter_size_column] : 1) + 2); })
      .on('mouseout', function () { d3.select(this).attr('opacity', 0.6).attr('r', d => sizeScale(config.scatter_size_column ? d[config.scatter_size_column] : 1)); });
    ...
    svg.append('text').attr('text-anchor', 'end').attr('x', width).attr('y', height + 35)
      .attr('fill', '#fff').attr('font-size', '12px').text(config?.scatter_x_column);
