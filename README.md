...
2026-09-08  OK. Complete-game enrichment (#341). New I/O, all inside existing facades and roots: bnetfacade gains FetchGameInfo (bridge GET, same loopback+/web-api/ guard and #319 budget as every bridge call) and HeadReplay/GCSReplayPath (HEAD against the one allowlisted GCS host+prefix; unmetered, bounded to a game's participant list); persist gains bnet_enrich/queue.json under the app-data root via iofacade with the same tmp+rename write as the other caches; the enrichment worker writes downloaded "<stem>-complete.rep" files into the already-allowlisted replay folder via iofacade (never overwriting), and on the Windows Low worker via a new place-replay broker op whose destination is validated to a bare "*-complete.rep" basename that must not exist; the see-replay broker op's destination grows from one fixed filename to an allowlist of two (watch_me.rep, watch_me_complete.rep). Downloads keep the existing seRS validation and download budget, plus an own persisted daily cap of 24 bridge calls with durable per-game stop conditions. No new roots, hosts, facade exemptions, enforcement-test changes, or DetectorVersion bump (parsing and detection unchanged; the "-complete" pairing is in-memory library state).
2026-09-07  OK. Session AKA pills and undetermined-winner third state. LibStore.HasLocalPlayers is a read-only in-memory lookup over the existing filtered view, no new os/net calls. bnetProfileToon gains a LocalPlayerKey field filled from that lookup in withBnetProfiles and buildWorkflowPlayerOverview; frontend replaces N client-side API calls with the pre-filled field. No new roots, hosts, endpoints, paths, facade exemptions, enforcement-test changes, or DetectorVersion bump.
