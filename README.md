...
[![Replay load throughput](https://img.shields.io/badge/replay%20load-26.2%20replays%2Fsec-brightgreen)](.github/workflows/bench-load.yml)

<!-- load-bench-start -->
<sub>38.10 ms/replay · corpus: 166 replays · GitHub-hosted 2-core runner · updated automatically on merge to main</sub>
<!-- load-bench-end -->
...
```
2026-09-04  OK. Dead weight removal, no behaviour change. Deletes the build-order research notes and a stale proposal from docs/, five investigation scripts with no callers (debug_phases, debug_polygons, measure_stacking_drop, scan_muta, scan_nydus), the dashboard and settings migration sets whose tables nothing reads since the dashboard moved to JSON files, the --clean-dashboard flag, and seven Storage methods with no callers. New databases now get only the replay tables; the settings and bnet_profiles tables of an existing one are deliberately left in place, because the dashboard imports them once on its first run and dropping them from under an ingest would lose the user's folder and their rate-limited profile cache. One new replay migration drops player_aliases, which nothing has read since aliases were retired. No I/O surface change: no new os/net calls, roots, hosts, endpoints, facade exemptions, enforcement-test change, or AlgorithmVersion bump.
2026-09-04  OK. Replay library tuning: a read is capped at the newest 500 replays (settings.json max_replays, no screen for it), the folder watcher keeps the games the cap excluded out via a modification-time cutoff while always taking newer ones, and the browser is told a read started and finished rather than after every batch. Two fixes: a folder change and the example replays loaded nothing because the load inherited the HTTP request's context and died when the response was written, and the watcher only reports a file as gone once it confirms with iofacade.Stat that it really left the disk. No new I/O capability: the same iofacade walk, stat and reads as before, one extra Stat per path missing from a walk, and nothing new is written. No new endpoints, hosts, roots, os/net calls, enforcement-test change, or AlgorithmVersion bump.
2026-09-04  REVIEW. The dashboard stops opening a database and answers every read from an in-memory replay library. New capability: github.com/fsnotify/fsnotify, wrapped as internal/iofacade.NewDirWatcher so every watched directory is resolved against the same allowlist as any other facade call, with a darwin stub that returns ErrWatchUnsupported (its kqueue backend opens one descriptor per file, which a large replay folder would exhaust) so macOS reconciles by walking instead. The watcher only reports paths and never opens a file. Reads stay on iofacade: fileops.ScanReplayFolder wraps the existing walk and drops the watch-me staging folder, hidden directories inside the replay folder, half-written files and LastReplay.rep, and the loader hashes and parses through fileops and the screp parser exactly as the CLI ingest does, under the same per-replay panic guard. Writes are new but confined to the app-data root, an existing permitted root, and all go through iofacade with a temp file plus rename: settings.json, one JSON file per cached Battle.net profile, and one per account's play history. internal/legacyimport opens the previous screp.db read-only (mode=ro) once, to carry those settings and caches over, and never writes to it. scripts/pro-pack keeps its own SQL-backed reader for offline pack building, which the new dashboard guard test exempts by name. screpdb ingest and screpdb mcp keep their SQLite paths untouched. No new roots beyond the replay folder the dashboard already had, no new os/net calls outside the facades, no enforcement-test change, no AlgorithmVersion bump.
2026-09-04  REVIEW. Replay library loader, folder watcher and one-shot legacy import, the file-reading half of moving the dashboard off SQLite. New capability: github.com/fsnotify/fsnotify, wrapped as internal/iofacade.NewDirWatcher so every watched directory is resolved against the same allowlist as every other facade call, with a darwin stub that returns ErrWatchUnsupported (its kqueue backend opens one descriptor per file, which a large replay folder would exhaust) so macOS reconciles by walking instead. The watcher only reports paths; it never opens a file. All reads stay on iofacade: fileops.ScanReplayFolder wraps the existing walk and drops the watch-me staging folder, hidden directories, half-written files and LastReplay.rep, and the loader hashes and parses through fileops and the screp parser exactly as the CLI ingest does, under the same per-replay panic guard. internal/legacyimport opens the existing app-data screp.db read-only (mode=ro) to copy the settings row and the Battle.net caches out once, and never writes to it. No new roots are registered beyond the replay folder the dashboard already had, no writes outside the app-data root, no os/net calls outside the facades, no enforcement-test change, no AlgorithmVersion bump.
2026-09-03  OK. Dead code removal ahead of the in-memory replay library (step 1 of removing SQLite from the dashboard). Deletes the unused executeQuery/withFilteredConnection local-filter path and its raw *sql.DB query/scan Store helpers, ten unreferenced Store query methods with their sqlc queries, the orphaned sqlcgen player_summary file, test-only player-insight helpers, and the empty /api/custom/global-replay-filter/options stub (spec removed, code regenerated). Tests now discover replay ids and player keys through the public API instead of raw SQL. Pure removal: no new I/O, endpoints, queries, os/net calls, facade changes, or AlgorithmVersion bump.
2026-09-02  OK. Players list omnibar + player summary list tweaks. FilterOmnibar is parameterized (axes, state labels, noun, and an optional live text filter) and the players list now uses it in place of its ad-hoc filter row, wired to the existing name/onlyFivePlus/lastPlayed filter state and /api/players parameters; no query or endpoint changes. The players table adopts the games list header and hairline styles. The player summary last-games rows gain a Time column and show the player-oriented matchup instead of 1v1; the ladder stat guards against missing wins/losses (kospetrov rendered "undefined-undefined"). Go changes are removals only: the [country-flags] debug log lines in internal/dashboard/bnet_profiles.go are deleted (no behavior change) and a counter loop is modernized to range-over-int. No new I/O, endpoints, queries, os/net calls, facade changes, or AlgorithmVersion bump.
2026-09-02  OK. Player Summary tab visual polish, frontend-only. The Battle.net and Local games sections gain emoji-titled headers and labeled stat tiles (battle tag and country rows dropped; EAPM folded into the APM tile), "Also plays as" aliases render as pills and link to the local player page when one exists (checked client-side via the existing GET /api/players/{key} endpoint, no new backend code), the last-games list becomes a single table with a header row, hairline row separators and a wider composition column with early/mid/late labels, the player title spacing is normalized, and chat summary messages render full-width. No Go changes at all: no new I/O, endpoints, queries, os/net calls, facade changes, or AlgorithmVersion bump.
2026-09-02  OK. Players page rework. Removed: the player Summary tab's per-matchup cards, special pills and distinctive-outlier machinery, the Recent games tab, and the player Back button, ripping out their endpoints (playerSummaryPerMatchup/Special/Outliers, playerOutliers, playerRecentGames), builders, payload types, eleven now-orphaned sqlc/store queries (player_summary.sql entirely, the outlier and population queries in player_insight_static.sql) and their tests. Added: playerLastGames (GET /api/players/{playerKey}/last-games), which reuses the recent-games query (now also selecting team_format, LIMIT 10) and per-item populates the requesting player's result, APM/EAPM (two columns added to an existing SELECT), a disconnect flag (one new read-only query over replay_events player_dropped rows), featuring patterns and the request-time unit composition already computed for the per-game page; CountPlayerBnetGames (one COUNT over replays+players) gating the new Battle.net summary section; and lifetime games/wins/losses/disconnects plus a derived average APM decoded from the already-cached bnet profile payload (no new fetches, cache-only read path unchanged). Frontend: players list pages like the games list, tab order Summary/Hotkeys/Skill proxies/Chat summary, and the new Summary tab renders Battle.net (hidden without bnet-sourced replays) and Local games with the last ten games as rows. All reads go through existing sqlc/store paths; no os/net calls, no facade changes, no enforcement-test change, no AlgorithmVersion bump (nothing persisted changed).
2026-09-02  OK. Hotkey "Add" semantics fix, detection-layer only. Investigating a player (Howard_The_Duck in the BGH fixture) who "used groups without ever assigning" showed shift+number ("Add", hotkey type 0x02) adds the current selection TO the group, creating it when empty; every selection replay in the codebase (internal/unittags Analyze/Coordinates/MorphSelectionSizes/DetectMutaHarass and the hotkeystream extractor) had it backwards, pulling the group into the selection. All five now mutate the group and leave the selection intact, and the extractor folds Add into an assign of the group's new contents so shift+number-only players get building labels and counts. Marker and parser goldens regenerated (their diffs are exactly the retired used_hotkey_groups rows plus the corrected Add attribution); the storage fixture count drops by the retired marker's rows. Pure in-memory analysis changes: no new I/O, endpoints, queries, os/net calls, or facade changes; AlgorithmVersion stays 68 (unreleased, its re-ingest carries this too).
```

```
2026-09-02  OK. Hotkey screens polish, purely presentational. Map composite: building sprites now keep their true aspect ratio (width fit to the footprint, standing up to twice its height, anchored bottom center) instead of being stretched into the footprint box; footprint outlines use one accent colour; the circled comma-joined group badge becomes a row of small keycap-style chips drawn above the sprite so it is never covered. Frontend: short games stretch the timeline to the full lane width (ResizeObserver-driven px-per-minute floor), map crops cap at 75vh with object-fit contain, and the tab note reuses the standard workflow-section-warning styling. All drawing stays in the existing in-memory composite; no new I/O, endpoints, queries, os/net calls, or facade changes; no AlgorithmVersion bump (stored data unchanged).
```

```
2026-09-02  OK. Hotkey screens iteration, all presentation and derived-data changes. Backend: hotkey_stream unit counts now cap at Brood War's 12-unit selection limit (extract.go); the used_hotkey_groups marker, its evaluator and its summary pill are retired (never_used_hotkeys stays), with ListHotkeyGamesRateByPlayer re-pointed from that marker to players.hotkey_stream IS NOT NULL, so the never-hotkeys player pill keeps working on the same read path; the hotkey map endpoint accepts an exact-second cutoff (same validation shape as minute) so the frontend can notch its slider at building-assign moments; ComputeSignature additionally aggregates per-key assign-minute spans. Frontend: timeline drops the legend, race chips, per-row summaries and hotkey-less players, repeats the minute axis per player, and states that unit types are examples; map crops render two per row with a notched slider that picks the richest moment per minute; signature cards drop title, prose, scores and labels, and render one amber assign bar and one blue use bar per key with instant tooltips. No new I/O: same tables, same endpoints (one new optional query param), no os/net calls, no iofacade/netfacade change, no enforcement-test change; AlgorithmVersion stays 68 (unreleased on this branch, its re-ingest carries the cap + marker retirement).
```

```
2026-09-02  OK. Test coverage lift to 82% (hand-written code, scripts/coverage.sh) plus one correctness fix the new tests caught: screp names the comsat Build unit "ComSat" and the queens nest "Queens Nest", so extract.go now canonicalizes screp build names before evidence/placement matching (without it, comsat add-on evidence and comsat/queens-nest tile locations silently never matched). All other changes are tests: a synthetic-replay harness for hotkeystream.Extract, unit tests for the hotkey map composite and its handler against the ingested test corpus, direct endpoint tests for gameHotkeys/playerHotkeySignature (including a temporarily renamed-and-restored row set in the shared in-memory test DB), feature-flag and gaming-session route tests, a read-only endpoint sweep over every corpus game and player, and seeded dashboard/db Store tests for the hotkey, gaming-session, workflow filter-count and bnet-profile queries. Tests write only to per-test temp DBs and the existing test fixtures; no new I/O paths, no os/net calls, no iofacade/netfacade change, no enforcement-test change, no AlgorithmVersion bump (68 already forces the re-ingest that carries the comsat fix).
```

```
2026-09-02  OK. Hotkey intel: timeline + map on the game report, signature on the player page. players.hotkey_stream moves to wire format v2 (internal/hotkeystream: second deltas, assigns annotated with unit counts or the proven building type + build tile), extracted in the parser from the raw screp stream (hotkeystream.Extract mirrors unittags' selection replay) and stored through the existing insertPlayersBatchTx path; AlgorithmVersion 67->68. Reads added: three sqlc queries over players/replays (per-replay streams, per-player streams by key, one player+replay stream), two strict-server JSON endpoints (gameHotkeys, playerHotkeySignature) that only decode stored blobs, and one custom PNG endpoint (/api/custom/hotkeys/map) that re-reads the already-ingested replay file for terrain via the same scmapanalyzer render, disk cache path, and singleflight key as the existing /api/custom/game-assets/map, then composites sprites in memory. All file access goes through iofacade/appdata paths that were already permitted; no new roots, no writes outside the existing game-assets cache, no os/net calls, no netfacade change, no enforcement-test change.
```

```
2026-09-01  OK. Filter omnibar for the games list (issue #359). Two read-only aggregate queries complete the option counts (Store.CountWorkflowMatchupGames and CountWorkflowMapKindGames, one GROUP BY over replays each, mirroring buildMatchupClause and buildMapKindClause so a count matches its filter), which makes a zero mean "none in your corpus" instead of "unknown". The rest is frontend: a new components/FilterOmnibar.jsx replaces two <select> dropdowns and forty flat filter pills with one typed input plus a browse row of seven axis blocks, reading the same /api/games filter_options payload and writing the same mainGamesFilters state, so no new endpoint, request or query parameter is involved. Chips are grouped by filter state key rather than display axis so the rendered OR/AND grammar matches BuildWorkflowGamesListWhere. Also aligns the games-list team blocks into one CSS grid per Players cell. No writes, no detection change, no AlgorithmVersion bump, no os/net calls, no iofacade/netfacade allowlist widening, no enforcement-test change.
```
