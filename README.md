...
<!-- IO-AUDIT:START -->
- **2026-07-02** — `OK`. Follow-up to #248: dashboard-frontend copy only — for package-manager (Homebrew/Scoop) installs the "update available" control now shows the exact upgrade command (`brew upgrade screpdb` / `scoop update screpdb`) in its label instead of hiding it in a tooltip. No Go changes, no os/net calls, no `iofacade`/`netfacade` allowlist widening, no enforcement-test changes.
