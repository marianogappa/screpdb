...
2026-09-12  OK. Last Gaming Session (#402). Pure logic split inside internal/dashboard/endpoint_gaming_session.go: gamingSessionWindow drops its recency early-return so it only reports the bounds of the most recent sitting, and two new pure predicates (gamingSessionIsLive, gamingSessionIsRecentEnough) decide liveness and a 90-day outer bound; the response gains has_session alongside active, and the nav entry and panel gate on it. The candidate scan is byte-for-byte the same call (ListRecentAutosaveGamesForPlayers over the in-memory library, same 300-row cap, same autosaveOnly path filter), so no new reads, roots, hosts, endpoints, facade exemptions, enforcement-test weakening, or DetectorVersion bump.
```
