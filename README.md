...
2026-09-15  OK. Winner determination is now specified, not just implemented. The elimination correction's four thresholds and its production-command set are exported from internal/parser so internal/spec/sections_results.go can read the live values, and three new SPECIFICATION.md sections state what each source of evidence can answer, the ordered decision procedure, and the elimination signature's gates. The spec guard test regenerates and diffs the document, and each section's Verify asserts against the real constants, so the prose cannot drift from the code. Documentation and identifier renames only: no behaviour change, no new reads, roots, hosts, endpoints, facade exemptions, enforcement-test weakening, or DetectorVersion bump.
2026-09-14  OK. Winner attribution no longer credits a coalition that was eliminated. Both winner paths (screp's Computed.WinnerTeam and DeriveWinnersFromFinalTopology) read "no Leave Game command" as "still alive", so a team wiped out in the final seconds - before the replay saver exits and stops the recording - was credited the win it actually lost. internal/parser/eliminated_winner.go overturns that from the command stream already in memory: a destroyed team cannot act after its last unit dies and cannot Build/Train/Upgrade once its production is gone, while the team that beat it keeps doing both until the recording ends. It fires only when the saver is not among the credited winners (his silence measures his own exit, not his team's), the credited side fell silent at least 3s before the last surviving rival, stopped producing at least 15s earlier, and had produced nothing for at least 20s. Reads no new field: one extra pass over data.Commands after the existing dedup step, no re-parse, no file or network access, and it is a no-op once DetectMassDisconnectEnd has cleared the winners. Calibrated and verified against 59 hand-labelled games plus a full sweep of a 3,364-replay local corpus: 24 games change, zero panics, and zero cases where a correct credit became wrong. No new reads, roots, hosts, endpoints, facade exemptions, enforcement-test weakening, or DetectorVersion bump - is_winner is derived at ingest and the aggregates DetectorVersion gates (APM, cadence, viewport) are untouched.
```
