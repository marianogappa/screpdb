...
<!-- IO-AUDIT:START -->
```
2026-09-14  OK. Bridge discovery skiplist backs off per port instead of one flat TTL (#424). The skiplist muted any loopback port that answered a probe as not-the-bridge for five minutes, on the premise that a listener never becomes the bridge in place; SC:R breaks that premise by binding its port at launch and serving /web-api/ appreciably later, so its own port was silenced for five minutes at startup and the app reported not_running with the game open. The mute now starts at 30s and doubles per consecutive verdict to the same 5m ceiling, and skiplist entries survive their own expiry (only a port that stops listening is forgotten) so repeat offenders still escalate. 401 stays BridgeOffline and is still never skiplisted. No change to sweep cadence, so no extra lsof spawns and no extra traffic to third-party daemons; the only added traffic is one probe per warming-up port. No new reads, roots, hosts, endpoints, facade exemptions, enforcement-test weakening, or DetectorVersion bump.
```
...
```
2026-09-14  OK. Dropped games in the session record, and one Battle.net answer serving every surface it observes. The drop flag existed on the compacted player record but had no reader, and it was inverted for the case that matters most: in a saver-disconnect replay (issue #358) the user's own client records no leave for them and invents a "Dropped" leave for everyone still in the game, so the one player who really dropped was the only one unflagged. models.ReplayData now carries the parser's existing saver-disconnect verdict into compaction (no new detection, no re-parse, no DetectorVersion bump: the same DetectMassDisconnectEnd call that already zeroes the winners), which stamps the saver and withholds the phantom cluster's flags while keeping the leave reasons the file actually wrote. SessionPlayerAPMRow then carries the corrected flag through the same in-memory pass that already returned APM and leave seconds, so the session summary holds a drop apart from a loss and from an unresolved game and keeps it out of the win rate's denominator. Measured on the live corpus: a sitting that read 1-3 with 4 unresolved now reads 1-3 with 1 unresolved and 3 drops. The synergy work adds two in-memory reads over stores already loaded and makes no request of its own: BnetGameArchive.LastGameByToons reads participation (every game record already names everyone in it, whichever account's fetch reported it) and BnetCache.GatewaysByToons reads the alternate accounts a fetched profile already listed. Both reduce outbound work rather than adding it — the regulars sweep now skips anyone an existing observation already covers as well as a fetch would, and the profile backfill asks the gateway we have evidence for before falling back to the blind five-gateway sweep. regularFreshness now counts a known game as dating the look that found it, which is a tightening, not a loosening: a game cannot be known before it is played. The liveness rows are now pushed rather than waited for: fetchAndCacheBnetProfile, the single place any fresh answer lands, announces it on the existing library-events WebSocket (a new "observation" message type on the server's one push channel, no new endpoint or socket), throttled to one broadcast per 2s with a trailing edge so a burst costs one refetch and the last answer of a sweep is never the one nobody hears. Misses and cache hits announce nothing. The page reloads the session silently in response, which keeps the panel mounted and swaps only changed rows; it cannot feed back, because the session endpoint reads profiles cache-only and the sweep it re-arms is gated by its own interval and in-flight flag. No new reads, roots, hosts, endpoints, facade exemptions, enforcement-test weakening, or DetectorVersion bump.
2026-09-13  OK. Players-list facets and session stat tiles. Three new players-list filters (race, APM band, games-played band) are computed by the same in-memory pass over player aggregates that already served the name and last-played filters (internal/dashboard/db/libstore_player_facets.go), with facet counts taken with each facet's own filter cleared so the bar never dead-ends; they are declared in api/openapi/dashboard.v1.yaml, regenerated, and added to the MCP read-only surface's param list. The redundant boolean 5+ games axis is retired from the UI while only_5_plus stays accepted by the API. Session tiles gain per-race APM and an undecided count, both derived from data already in the response, and the win rate's denominator becomes every game of the sitting. No new reads, roots, hosts, endpoints, facade exemptions, enforcement-test weakening, or DetectorVersion bump.
2026-09-13  OK. Regulars freshness is now dated by its own observation. internal/dashboard/session_regulars.go reads BnetProfile.FetchedAt (already in the distilled profile store, no new read) alongside the archived game times, and regularFreshness takes that observation age as a second axis: the live tier needs an answer newer than regularsObservationWindow, and past regularsObservationMaxAge (two sweeps, not two days) the row is dropped rather than asserted, because a stale look omits everyone who started playing since and so yields a biased list rather than merely an old one. Keying off when Battle.net last answered, never off bridge connection state, means a brief disconnect changes nothing and a long silence degrades smoothly. A failed sweep leaves FetchedAt untouched, so it correctly never counts as an observation. No change to polling volume (same 30-minute sweep, same paced facade), and no new roots, hosts, endpoints, facade exemptions, enforcement-test weakening, or DetectorVersion bump.
2026-09-13  OK. Session players table: P/W/L split, sortable columns, identity badge column. Frontend-only except one field: gamingSessionPlayer now carries PrimaryBadge, filled by the existing in-memory d.primaryIdentityBadge resolver (memoised fingerprint match plus the embedded scfingerprint registry), the same call the games and players list endpoints already make. Sorting is client side over the rows already sent. No reads, writes, roots, hosts, endpoints, facade exemptions, enforcement-test weakening, or DetectorVersion bump.
2026-09-12  OK. Session regulars and their Battle.net freshness. The ranked, account-merged co-player list is computed from the in-memory library (new LibStore.ListCoPlayerCounts, one pass over the same snapshot every other list endpoint reads) and annotated from data already on disk: the distilled profile store for account identity and the game archive for each account's newest published game. The only new outbound work is a freshness sweep that refetches at most regularsMax profiles, at most once every 30 minutes, through the existing rate-limited bnetfacade bridge client at PriorityBackground, on the exact (toon, gateway) pairs already cached, so it opens no new host, path or gateway-guessing behaviour and is paced by the same token bucket as every other bridge call. The goroutine is crashreport.GuardNonFatal-guarded per the panic architecture. Time-in-game now sums the user's own leave stamps rather than replay length, which is a correctness fix, not I/O. No new roots, hosts, endpoints, facade exemptions, enforcement-test weakening, or DetectorVersion bump.
```
