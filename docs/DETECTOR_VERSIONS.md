...
build-order overhaul — Zerg 5/6/7/8/10/11 Pool rungs, loosened FFE &
...
Build dedup now requires the same build tile, not just a 3s window —
...
Selection-tag build dedup (internal/unittags + internal/builddedup),
...
Terran build-order revamp (issue #155). The topology openers 1 Rax 1 Fac
...
Expert milestone timings for the composition-based Terran BOs (issue
...
Coordinate enrichment (issue #175). Production / research / cancel
...
Preferred build-order tier. Specific, scene-named openers (e.g. "3 Hatch
...
Terran composition + cliff-drop accuracy fixes. (a) The Goliath opener now
...
Coordinate-unit normalization. Build commands carry TILE-unit
...
1v1 attack detection rewritten (issue #186). For exactly-two-opposing-
...
Removed dt_drop classification (issue #185). "DT produced near a drop"
...
Removed reaver_drop classification too (issue #185 follow-up). Reavers
...
Offensive-nydus detection (issue #193). A forward BuildNydusExit placed in
...
Mutalisk hit-and-run detection (issue #194). Selection/hotkey state is
...
Build-order accuracy + modifiers. Multi-larva Zerg morphs are now counted
...
Bio is classified by base count (1-Base / 2-Base, keys bo_t_bio_1base /
...
build-dedup (Tier B never-produced) now collapses same-tile Build commands
...
earlyfilter backtrack no longer force-drops income-producing worker trains
...
Terran air/specialist openers (#228). The TvZ "Wraith" composition opener
...
new strategic markers — Made Maelstrom (PvZ Dark Archon cast), Crazy Zerg
...
Zerg pool/hatch supply-count fix. ProduceCountBeforeBuild now counts
...
add the 13 Hatch hatch-first rung (9 Drone morphs + Overlord before the
expansion Hatchery) — previously fell into the Pool/Hatch (Other) residual.

## 49

3 Hatch Muta converted from a build-order opener to a TvZ composition
marker (key three_hatch_muta) so the hatch-first opener underneath (11/12
Hatch) surfaces on its own.

## 50

fuzzy Zerg opener (bo_z_fuzzy). When a multi-unit-selection Drone morph
...
Terran mech taxonomy reformulated (#226/#227). Mech is now named by the
...
round-10 curation. "Mech (no expa)" family renamed to "1-Base Mech" /
...
N Hatch Hydra counts bases up to the 1st Hydralisk + a 30s grace window
...
"N Hatch <tech>" redesign (issue #245). Hydra/Muta/Lurker are now
...
Team-stacking flag now uses a shorter 2-minute floor for a stacking band
...
build-dedup no longer drops an expansion Command Center (or any building)
...
Speedlot timing reports Leg Enhancement research COMPLETION (start +
...
Wraith Cloak timing reports Cloaking Field research COMPLETION (start +
...
larva morphs cancelled before the first Overlord are dropped from the
...
the early-game economy sim now reverses a cancelled Build (Cancel Build) —
...
ingest now extracts and stores scfingerprint feature vectors per human
...
scmapanalyzer bumped to 2026-08-16, which fixes base recognition on the
...
money-map classification now uses the median mineral field instead of
...
the never_upgraded / never_researched per-matchup suppression floors are
...
16 when pros prove research by ~2:46. The floors gate ShouldSave, so
...
mass-disconnect games no longer credit the replay saver's team a phantom
